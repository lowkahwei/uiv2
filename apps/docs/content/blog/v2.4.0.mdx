...
> If you are using it from `@sytechui/react`, no changes are required.

The new `cn` utility exported from the `@sytechui/theme` package includes `tailwind-merge` to avoid conflicts between tailwindcss classes overrides and includes the config for NextUI custom classes.

If you are using the `cn` utility from the `@sytechui/system`,

```diff-tsx
-import {cn} from "@sytechui/system"
```

or `@sytechui/system-rsc` package,

```diff-tsx
-import {cn} from "@sytechui/system-rsc"
```
