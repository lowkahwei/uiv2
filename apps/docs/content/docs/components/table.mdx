...
across those columns. Use `fullWidth={false}` to preserve the exact declared widths. Put
`Table.Footer` inside `Table.Content` and compose it from `Table.Row` and `Table.Cell`. React Aria
then renders a native `<tfoot>` for non-virtualized tables, so header, body, and footer share the
browser's column tracks without a second width calculation.
...
- `Table.Footer`: React Aria's footer row group. Place it inside `Table.Content` after
  `Table.Body`, and render `Table.Row` / `Table.Cell` children. It is a native `<tfoot>` for
  non-virtualized tables and supports `isSticky`.
