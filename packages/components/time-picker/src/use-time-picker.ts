...
import { CalendarDate, CalendarDateTime, DateValue, getLocalTimeZone } from "@internationalized/date";
import { useDateFormatter } from "@react-aria/i18n";
import { TimePickerValue, UseTimePickerOptions, TimePickerItem, TimeUnit } from "./types";
...
export const useTimePicker = (options: UseTimePickerOptions = {}) => {
  ...
  const handleIndexChange = useCallback(
    (unit: TimeUnit, index: number) => {
      ...
      let derivedDateValueForOnChange: DateValue | undefined; 
      ...
      setInternalState(current => {
        ...
        derivedDateValueForOnChange = stateToDateValue(updated, enabledUnits);
        return updated;
      });
      if (derivedDateValueForOnChange && onChange) {
        onChange(derivedDateValueForOnChange);
      }
    },
    [ranges, enabledUnits, onChange]
  );
