...
/** Pinning metadata is consumed only by the native table geometry container. */
export type ColumnPinnedSide = "start" | "end";

export interface ColumnGeometryDefinition {
  id: Key;
  flex?: number;
  maxWidth?: number;
  minWidth?: number;
...
export interface ColumnGeometry {
  byId: Map<Key, ResolvedColumnGeometry>;
  center: ResolvedColumnGeometry[];
  columns: ResolvedColumnGeometry[];
  end: ResolvedColumnGeometry[];
  hasHorizontalOverflow: boolean;
  hasValidPinnedWidth: boolean;
  pinnedEndWidth: number;
  pinnedStartWidth: number;
  start: ResolvedColumnGeometry[];
  totalWidth: number;
  viewportWidth: number;
  widths: Map<Key, number>;
}

export interface PinnedShadowColumns {
  end?: ResolvedColumnGeometry;
  start?: ResolvedColumnGeometry;
}
...
export function calculateColumnGeometry(
  definitions: readonly ColumnGeometryDefinition[],
  viewportWidth: number,
  ...
  let start = 0;
  const columns = resolved.map<ResolvedColumnGeometry>((column, index) => {
    const result = {
...
export function getPinnedShadowColumns(
  geometry: ColumnGeometry,
  scrollOffset: number,
): PinnedShadowColumns {
  if (!geometry.hasHorizontalOverflow || geometry.viewportWidth <= 0) return {};

  const start = geometry.start.reduce<ResolvedColumnGeometry | undefined>((frontier, column) => {
  ...
  const end = geometry.end.reduce<ResolvedColumnGeometry | undefined>((frontier, column) => {
    const isSticky =
