...
describe("Sidebar", () => {
  ...
  it("keeps labels mounted while the sidebar width collapses", async () => {
    const user = userEvent.setup();
    ...
    const groupLabel = getByRole("heading", {name: "Main"});

    await user.click(getByRole("button", {name: "Collapse sidebar"}));

    expect(container.querySelector('[data-slot="label"]')).toBe(label);
    expect(label).toHaveClass("flex-1");
    expect(label).not.toHaveClass("w-0");
    expect(container).toContainElement(groupLabel);
    expect(groupLabel).toHaveAttribute("aria-hidden", "true");
  });

  it("keeps custom transition timing synchronized with the width container", () => {
    const {getByRole} = render(
      <SidebarProvider>
        <Sidebar
          style={{"--sidebar-duration": "600ms", "--sidebar-ease": "linear"} as React.CSSProperties}
        />
      </SidebarProvider>,
    );
    const container = getByRole("complementary").parentElement;

    expect(container).toHaveStyle({"--sidebar-duration": "600ms", "--sidebar-ease": "linear"});
  });
